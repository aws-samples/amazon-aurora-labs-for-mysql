...
1. You are now creating an Aurora Global Database, adding a new region DB cluster to be replicated from your primary region's Aurora DB cluster.
   ...
   !!! warning "Validate Settings" Before moving on, <span style="color:red;">**please re-validate all your settings**</span>, anything that's not explicitly called out in the instructions here can be left on the default values. Remember, some database settings and configurations are immutable after creation. 
   
1. After confiring carefully that we have everything in order, press the **Add Region** button.
